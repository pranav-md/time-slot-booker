...
export function combineDateTime(dateTime, time) {
  ...
  export function formattedEventList(events){
    const formattedList =  events.map(item => {
        return {timeToDisplay: DateTime.fromISO(item[0], { setZone: true }).toFormat("MMMM dd, yyyy 'at' h:mm a ZZZZ"), time: item[0]}
      });
return formattedList
  }

  export function deserialiseStringToDateTime(dateTime){
    const timeSerialized = DateTime.fromFormat(dateTime, "MMMM dd, yyyy 'at' h:mm a ZZZZ");
  ...
  export function formatEventTimes(events) {
    return events.map(event => {
        ...
        const endFormatted = end.toFormat("EEEE, MMMM dd, yyyy 'at' HH:mm '  Timezone:'ZZZZ");
